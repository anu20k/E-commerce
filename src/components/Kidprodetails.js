...
export default function Kidprodetails({addToCart}) {
    let param = useParams()
  let { title } = param

  const data = Kidsdata.find((item) => item.title === title)
  return (
    <div>
      ...
      <Container>
        <Row className="py-5 ">
          ...
          <Col className='py-5 '>
            ...
            {/* <span className='fw-bold fs-5 text-pink'>({data.offer} OFF)</span> */}
